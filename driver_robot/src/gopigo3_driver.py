...
class Robot:
    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.publish_motor_status(self.ML, 'left')
            self.publish_motor_status(self.MR, 'right')

            # publish current pose
            (odom, transform) = self.odometry()
    ...
    def odometry(self):
        # Get encoder values for left and right motors
        encoder_l = self.g.get_motor_encoder(self.ML)
        encoder_r = self.g.get_motor_encoder(self.MR)
    
        # Calculate linear and angular speed from encoder values
        lspeed = encoder_l / 360.0 * self.CIRCUMFERENCE
        rspeed = encoder_r / 360.0 * self.CIRCUMFERENCE
        twist = TwistWithCovariance()
        twist.twist.linear.x = (rspeed + lspeed) / 2.0
        twist.twist.angular.z = (rspeed - lspeed) / self.WIDTH
        # Update robot's pose based on odometry calculations
        # (Implement your odometry calculation logic here)
    ...
    def publish_motor_status(self, motor, side):
        (flags, power, encoder, speed) = self.g.get_motor_status(motor)
        status = MotorStatus(low_voltage=(flags & (1<<0)), overloaded=(flags & (1<<1)),
                              power=power, encoder=encoder, speed=speed)
        if side == 'left':
            self.pub_enc_l.publish(Float64(data=encoder))
            self.pub_motor_status.publish(MotorStatusLR(header=Header(stamp=rospy.Time.now()), left=status))
        elif side == 'right':
            self.pub_enc_r.publish(Float64(data=encoder))
            self.pub_motor_status.publish(MotorStatusLR(header=Header(stamp=rospy.Time.now()), right=status))
